...
const BudgetGenerator: React.FC<BudgetGeneratorProps> = ({ products, settings, onSave, initialBudget, onCancel }) => {
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      client, items, requiredMaterials, taxRate, discount, subtotal, total, status: initialBudget ? initialBudget.status : 'pendiente'
    });
  };
  ...
  return (
    <div className="flex flex-col lg:grid lg:grid-cols-4 gap-6 animate-slide-up pb-24">
      <div className="lg:col-span-3 space-y-6">
        ...
        {/* CABECERA EDICION */}
        {initialBudget && (
          <div className="bg-amber-500 p-4 rounded-2xl flex items-center justify-between shadow-lg">
            <div className="flex items-center gap-3 text-slate-900">
               <Edit2 size={20} />
               <span className="text-xs font-black uppercase italic tracking-tighter">Modificando Expediente: {budgetID}</span>
            </div>
            <button onClick={onCancel} className="p-2 bg-slate-900 text-amber-500 rounded-xl hover:scale-105 transition-transform"><X size={16}/></button>
          </div>
        )}
        ...
        {/* BUSCADOR DE RUBROS */}
        <section className="bg-slate-950 p-6 rounded-[2rem] shadow-xl relative overflow-visible">
          ...
          <div className="flex flex-col gap-4 relative z-20">
            <div className="relative">
              <input 
                ...
                className="w-full bg-slate-900 text-white px-5 py-4 rounded-xl border-2 border-slate-800 focus:border-amber-500 outline-none font-bold uppercase italic text-sm placeholder:text-slate-600" 
              />
              {searchTerm && !selectedProductId && (
                <div className="absolute top-full left-0 right-0 z-[100] mt-2 bg-white rounded-2xl shadow-2xl max-h-72 overflow-y-auto border-2 border-slate-900 custom-scrollbar">
                  {filteredProducts.length > 0 ? (
                    filteredProducts.map(p => (
                      <button key={p.id} onClick={() => { setSelectedProductId(p.id); setSearchTerm(p.name); }} className="w-full text-left p-4 hover:bg-amber-50 border-b border-slate-100 last:border-0 flex justify-between items-center group transition-colors">
                        <div>
                        ...
                        <CheckCircle2 size={16} className={`shrink-0 ${selectedProductId === p.id ? 'text-amber-500' : 'text-slate-200'}`} />
                      </button>
                    ))
                  ) : (
                    <div className="p-8 text-center text-slate-400 uppercase font-black text-[10px] tracking-widest italic">No se hallaron rubros</div>
                  )}
                </div>
              )}
            </div>
            <div className="flex gap-3">
              ...
              <button onClick={addItem} disabled={!selectedProductId} className="flex-[2] bg-amber-500 text-slate-950 font-black rounded-xl shadow-lg disabled:opacity-20 uppercase italic tracking-tighter text-sm active:scale-95 transition-transform">
                CARGAR A PLANILLA
              </button>
            </div>
          </div>
        ...
        {/* LISTADO DE ITEMS CON EDICIÓN EN LÍNEA */}
        <div className="space-y-3">
          <div className="flex items-center justify-between px-2">
             <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Detalle de Rubros (Toca valores para negociar)</h4>
             <span className="text-[10px] font-black text-amber-600 bg-amber-50 px-2 py-0.5 rounded-lg">{items.length} Tareas</span>
          </div>
          ...
          <div className="bg-white rounded-3xl border-2 border-slate-200 overflow-hidden shadow-sm">
            <table className="w-full text-left">
              <thead className="bg-slate-950 text-[9px] font-black text-amber-500 uppercase tracking-[0.2em]">
                <tr>
                  <th className="px-6 py-4">Rubro / Concepto</th>
                ...
                {items.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-5">
                      <p className="font-black text-slate-900 uppercase italic text-xs leading-tight">{item.name}</p>
                      <p className="text-[8px] text-slate-400 font-bold uppercase mt-1">Ref: {item.unit}</p>
                    </td>
                    <td className="px-6 py-5 text-center">
                      <input 
                        ...
                        className="w-16 bg-slate-50 border border-slate-200 rounded-lg py-1 px-2 text-center text-xs font-black outline-none focus:border-amber-500"
                      />
                    </td>
                    <td className="px-6 py-5 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <span className="text-[10px] font-black text-slate-400">{settings.currency}</span>
                        <input 
                          type="number" 
                          value={item.price} 
                          onChange={(e) => updateItemValue(idx, 'price', e.target.value)} 
                          className="w-24 bg-slate-50 border border-slate-200 rounded-lg py-1 px-2 text-right text-xs font-black outline-none focus:border-amber-500"
                        />
                      </div>
                    </td>
                    <td className="px-6 py-5 text-right font-black text-slate-900 italic text-sm">{settings.currency}{item.subtotal.toLocaleString()}</td>
                    <td className="px-6 py-5">
            ...
            {items.length === 0 && (
              <div className="py-16 text-center text-slate-300 italic flex flex-col items-center">
                 <Hammer size={32} className="opacity-10 mb-2" />
                 <p className="text-[9px] font-black uppercase tracking-widest">Planilla vacía</p>
              </div>
            )}
          </div>
        </div>
        ...
        <section className="bg-slate-100 p-6 rounded-3xl border-2 border-dashed border-slate-200">
          <h3 className="text-sm font-black text-slate-900 uppercase italic tracking-tighter mb-4 flex items-center gap-2">
            <Package size={16} /> Insumos de Campo
          </h3>
          <div className="flex gap-2 mb-4">
            <input type="text" value={matName} onChange={(e) => setMatName(e.target.value)} placeholder="MATERIAL..." className="flex-1 bg-white px-4 py-3 rounded-xl border border-slate-200 outline-none font-bold text-xs uppercase" />
            <input type="text" value={matQty} onChange={(e) => setMatQty(e.target.value)} placeholder="CANT." className="w-24 bg-white px-2 py-3 rounded-xl border border-slate-200 outline-none font-bold text-center text-xs" />
      ...
      <div className="lg:col-span-1">
        <div className="bg-slate-950 text-white p-6 rounded-[2.5rem] shadow-2xl border-t-[10px] border-amber-500 space-y-6 lg:sticky lg:top-24">
          <h3 className="text-lg font-black flex items-center gap-3 uppercase italic tracking-tighter text-amber-500">
            <Calculator size={20} /> Liquidación
          </h3>
          <div className="space-y-4">
            <div className="flex justify-between border-b border-white/5 pb-2">
              <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Subtotal Obra</span>
              <span className="text-white font-mono font-black">{settings.currency}{subtotal.toLocaleString()}</span>
            </div>
            <div className="space-y-2">
               <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Días Validez</span>
               <input type="number" value={validDays} onChange={(e) => setValidDays(parseInt(e.target.value) || 0)} className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 font-black text-amber-500 text-sm" />
            </div>
            <div className="flex items-center justify-between">
               <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Bonif. %</span>
               <input type="number" value={discount} onChange={(e) => setDiscount(parseFloat(e.target.value) || 0)} className="w-20 bg-slate-900 border border-slate-800 rounded-lg p-2 text-right font-black text-amber-500 text-xs" />
            </div>
            <div className="flex items-center justify-between">
               <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">IVA/Gastos %</span>
               <input type="number" value={taxRate} onChange={(e) => setTaxRate(parseFloat(e.target.value) || 0)} className="w-20 bg-slate-900 border border-slate-800 rounded-lg p-2 text-right font-black text-amber-500 text-xs" />
            </div>
            <div className="pt-4 text-right">
              <p className="text-[9px] font-black text-amber-600 uppercase italic">Inversión Final</p>
              <p className="text-4xl font-black text-white italic font-mono tracking-tighter leading-none">{settings.currency}{total.toLocaleString()}</p>
            </div>
          </div>
          <button onClick={handleSave} className="w-full bg-amber-500 text-slate-950 font-black py-5 rounded-2xl shadow-xl flex items-center justify-center gap-3 uppercase italic tracking-tighter text-base active:scale-95 transition-transform border-b-4 border-amber-700">
            <Save size={20} />
            {initialBudget ? 'ACTUALIZAR OBRA' : 'EMITIR COTIZACIÓN'}
          </button>
        </div>
      </div>
    </div>
